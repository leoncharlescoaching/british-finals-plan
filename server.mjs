  ...
  return async(req,res)=>{
    ...
    try {
      if(url.pathname==='/api/subscribe') {
        ...
        if(env.MAILCHIMP_TRANSACTIONAL_KEY && env.FROM_EMAIL) {
          try {
            const result=await provider('https://mandrillapp.com/api/1.0/messages/send.json',{
              key:env.MAILCHIMP_TRANSACTIONAL_KEY,
              message:{from_email:env.FROM_EMAIL,from_name:'Leon Charles | Look Good Fitness',subject:'Your British Finals training plan',to:[{email,type:'to'}],track_opens:false,track_clicks:false,
                text:`Here’s the plan you asked for, brother.\n\nThe exact 5-day split I’m running for my 2026 British Finals prep.\n\nDownload your copy: ${base}${downloadUrl}\n\nThis link works for 7 days. After that, request a fresh copy from ${base}.\n\nRead the execution rules first. If five days doesn’t fit your week, take the standards into a plan that does.\n\nLeon\nLook Good Fitness\n\nYou received this because you requested the guide. This request does not sign you up for other marketing emails. For privacy or delivery questions: leon@lookgoodfitness.co.uk`}
            });
            emailQueued=Array.isArray(result)&&result.some(x=>['sent','queued','scheduled'].includes(x.status));
          }catch{}
        }
        return json(res,200,{downloadUrl,emailQueued});
      }
      ...
      const files={'/':'index.html','/index.html':'index.html','/styles.css':'styles.css','/app.js':'app.js','/assets/leon.webp':'assets/leon.webp','/assets/logo.png':'assets/logo.png','/assets/guide-cover.webp':'assets/guide-cover.webp','/assets/favicon.svg':'assets/favicon.svg','/assets/anton.ttf':'assets/anton.ttf'};
